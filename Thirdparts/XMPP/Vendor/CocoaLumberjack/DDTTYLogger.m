...
+ (NSUInteger)codeIndexForColor:(OSColor *)inColor
{
	...
	for (OSColor *color in colors)
	{
		...
		DLogVerbose(@"DDTTYLogger: %3lu : %.3f,%.3f,%.3f & %.3f,%.3f,%.3f = %.6f",
		...
		if (distance < lowestDistance)
		{
			...
			DLogVerbose(@"DDTTYLogger: New best index = %lu", (unsigned long)bestIndex);
		}
...
+ (void)initialize
{
	...
	if (!initialized)
	{
		...
		DLogInfo(@"DDTTYLogger: isaColorTTY = %@", (isaColorTTY ? @"YES" : @"NO"));
		DLogInfo(@"DDTTYLogger: isaColor256TTY: %@", (isaColor256TTY ? @"YES" : @"NO"));
		DLogInfo(@"DDTTYLogger: isaXcodeColorTTY: %@", (isaXcodeColorTTY ? @"YES" : @"NO"));
...
- (void)setForegroundColor:(OSColor *)txtColor backgroundColor:(OSColor *)bgColor forFlag:(int)mask context:(int)ctxt
{
	dispatch_block_t block = ^{ @autoreleasepool {
		...
		DLogInfo(@"DDTTYLogger: newColorProfile: %@", newColorProfile);
...
- (void)logMessage:(DDLogMessage *)logMessage
{
	...
	if (logMsg)
	{
		...
		{
			// The log message is unformatted, so apply standard DLog style formatting.

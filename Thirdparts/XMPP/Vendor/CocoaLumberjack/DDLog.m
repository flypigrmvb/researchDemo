...
// 
// So we use a primitive logging macro around DLog.
// We maintain the NS prefix on the macros to be explicit about the fact that we're using DLog.

#define DD_DEBUG NO

#define DLogDebug(frmt, ...) do{ if(DD_DEBUG) DLog((frmt), ##__VA_ARGS__); } while(0)
...
+ (void)initialize
{
	...
	if (!initialized)
	{
		...
		DLogDebug(@"DDLog: Using grand central dispatch");
		...
		DLogDebug(@"DDLog: numProcessors = %u", numProcessors);
...
+ (void)lt_removeLogger:(id <DDLogger>)logger
{
	...
	if (loggerNode == nil)
	{
		DLogDebug(@"DDLog: Request to remove logger which wasn't added");
		return;
	}

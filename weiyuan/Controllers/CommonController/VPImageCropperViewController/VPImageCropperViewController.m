...
@property (nonatomic, strong) ImageCompletedBlock completionBlock;
...
- (void)viewDidLoad {
    [super viewDidLoad];
    [self initView];
    [self setRightBarButton:@"确定" selector:@selector(confirm:)];
    if (!_showTitle) {
        _showTitle = @"裁剪图片";
    }
...
- (void)initView {
    ...
    self.ratioView.layer.borderColor = kbColor.CGColor;
...
- (void)confirm:(id)sender {
    if (_completionBlock) {
        _completionBlock(YES, [self getSubImage]);
        [self popViewController];
    }
}
...
- (CGRect)handleBorderOverflow:(CGRect)newFrame {
    ...
    } else if (CGRectGetMaxX(newFrame) < self.cropFrame.origin.x + self.cropFrame.size.width) {
    ...
    } else if (CGRectGetMaxY(newFrame) < self.cropFrame.origin.y + self.cropFrame.size.height) {

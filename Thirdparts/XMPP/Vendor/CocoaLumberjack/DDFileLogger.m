...
// 
// So we use primitive logging macros around DLog.
// We maintain the NS prefix on the macros to be explicit about the fact that we're using DLog.

#define LOG_LEVEL 2

#define DLogError(frmt, ...)    do{ if(LOG_LEVEL >= 1) DLog((frmt), ##__VA_ARGS__); } while(0)
#define DLogWarn(frmt, ...)     do{ if(LOG_LEVEL >= 2) DLog((frmt), ##__VA_ARGS__); } while(0)
#define DLogInfo(frmt, ...)     do{ if(LOG_LEVEL >= 3) DLog((frmt), ##__VA_ARGS__); } while(0)
#define DLogVerbose(frmt, ...)  do{ if(LOG_LEVEL >= 4) DLog((frmt), ##__VA_ARGS__); } while(0)
...
- (id)initWithLogsDirectory:(NSString *)aLogsDirectory
{
	if ((self = [super init]))
	{
		...
		DLogVerbose(@"DDFileLogManagerDefault: logsDirectory:\n%@", [self logsDirectory]);
		DLogVerbose(@"DDFileLogManagerDefault: sortedLogFileNames:\n%@", [self sortedLogFileNames]);
	}
	return self;
}
                       ...
                       context:(void *)context
{
	...
	{
		DLogInfo(@"DDFileLogManagerDefault: Responding to configuration change: maximumNumberOfLogFiles");
...
- (void)deleteOldLogFiles
{
	DLogVerbose(@"DDLogFileManagerDefault: deleteOldLogFiles");
	...
	for (i = maxNumLogFiles; i < count; i++)
	{
		...
		DLogInfo(@"DDLogFileManagerDefault: Deleting file: %@", logFileInfo.fileName);
...
- (NSString *)logsDirectory
{
	...
	if (![[NSFileManager defaultManager] fileExistsAtPath:_logsDirectory])
	{
		...
		{
			DLogError(@"DDFileLogManagerDefault: Error creating logsDirectory: %@", err);
		}
	}
...
- (NSString *)createNewLogFile
{
	...
	do
	{
		...
		{
			DLogVerbose(@"DDLogFileManagerDefault: Creating new log file: %@", fileName);
...
- (void)scheduleTimerToRollLogFileDueToAge
{
	...
	DLogVerbose(@"DDFileLogger: scheduleTimerToRollLogFileDueToAge");
	
	DLogVerbose(@"DDFileLogger: logFileCreationDate: %@", logFileCreationDate);
	DLogVerbose(@"DDFileLogger: logFileRollingDate : %@", logFileRollingDate);
...
- (void)rollLogFileNow
{
	DLogVerbose(@"DDFileLogger: rollLogFileNow");
...
- (void)maybeRollLogFileDueToAge
{
	...
	{
		DLogVerbose(@"DDFileLogger: Rolling log file due to age...");
...
- (void)maybeRollLogFileDueToSize
{
	...
	if (maximumFileSize > 0)
	{
		...
		{
			DLogVerbose(@"DDFileLogger: Rolling log file due to size (%qu)...", fileSize);
...
- (DDLogFileInfo *)currentLogFileInfo
{
	if (currentLogFileInfo == nil)
	{
		...
		if ([sortedLogFileInfos count] > 0)
		{
			...
			{
				DLogVerbose(@"DDFileLogger: Resuming logging with file %@", mostRecentLogFileInfo.fileName);
...
- (NSDate *)creationDate
{
	if (creationDate == nil)
	{
		...
		{
			DLogError(@"DDLogFileInfo: creationDate(%@): getattrlist result = %i", self.fileName, result);
		}
...
{
	return [@{@"filePath": self.filePath?self.filePath:@"error",
		@"fileName": self.fileName?self.fileName:@"error",
		@"fileAttributes": self.fileAttributes?self.fileAttributes:@"error",
		@"creationDate": self.creationDate?self.creationDate:@"error",
		@"modificationDate": self.modificationDate?self.modificationDate:@"error",
...
- (void)renameFile:(NSString *)newFileName
{
	...
	if (![newFileName isEqualToString:[self fileName]])
	{
		...
		DLogVerbose(@"DDLogFileInfo: Renaming file: '%@' -> '%@'", self.fileName, newFileName);
		
		NSError *error = nil;
		if (![[NSFileManager defaultManager] moveItemAtPath:filePath toPath:newFilePath error:&error])
		{
			DLogError(@"DDLogFileInfo: Error renaming file (%@): %@", self.fileName, error);
		}
...
- (void)addExtendedAttributeWithName:(NSString *)attrName
{
	...
	if (result < 0)
	{
		DLogError(@"DDLogFileInfo: setxattr(%@, %@): error = %i", attrName, self.fileName, result);
	}
}
...
- (void)removeExtendedAttributeWithName:(NSString *)attrName
{
	...
	{
		DLogError(@"DDLogFileInfo: removexattr(%@, %@): error = %i", attrName, self.fileName, result);
	}
}
